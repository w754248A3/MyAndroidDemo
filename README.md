...
应用内置了一个简单的 UI 界面，支持跨存储提供者的文件和文件夹复制，并且复制完成后会保留源文件：
- **选择源文件或源文件夹**: 支持从系统内任意存储位置选择单个文件，或选择一个文件夹作为复制源（包括本应用私有存储）。
- **选择目标目录**: 支持选择任意文件夹作为存放位置。
- **执行复制**: 异步执行复制；文件保持原有逻辑，文件夹会递归复制其内部全部子文件和子目录。
...
3. 应用主界面提供了简单的按钮，用于在不同存储位置之间复制文件或文件夹，复制完成后不会删除源文件。
